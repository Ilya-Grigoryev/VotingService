...
<div>
  <v-card  class="mx-auto pa-3 ma-3"
      ...
      width="65%">
    <v-app-bar
        :src="image_url === 'null' || image_url === '/media/null' ?
   ...
   <v-dialog v-model="dialog" persistent max-width="800px">
          <template v-slot:activator="{ on, attrs }">
              <v-btn
                  @click="dialog = true"
                  dark
                  :disabled="voting.author.id !== user.id || voting.status !== 'not started'"
                  color="orange"
    ...
    <v-btn
        @click="end_vote()"
        dark
        :disabled="voting.author.id !== user.id || (voting.status !== 'active' && voting.status !== 'infinite')"
        color="red darken-3"
    ...
    export default {
      ...
      methods: {
        ...
        save_changes() {
          ...
          this.axios.post(
              `http://localhost:8000/api/change_poll/${this.$route.params.id}/`,
              formData,
              {
        ...
        get_poll() {
          ...
          .then(response => {
            ...
            for (let i = 0; i < answers.length; i++)
                this.options.push(answers[i].text)
